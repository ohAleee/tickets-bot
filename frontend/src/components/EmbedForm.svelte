    ...
    <form class="form-wrapper" on:submit|preventDefault>
        ...
        <div class="row">
            <Textarea
                col1
                label="Description"
                placeholder="Large text area, up to 4096 characters"
        ...
        <Collapsible defaultOpen>
            ...
            <div slot="content" class="row">
                <Input
                    ...
                    bind:value={data.author.name}
                />
                <Input
                    col3
                    label="Author Icon URL (Optional)"
                    placeholder="https://example.com/image.png"
                    tooltipText="Small icon displayed in the top left"
                    bind:value={data.author.icon_url}
                />
                <Input
                    col3
                    label="Author URL (Optional)"
                    placeholder="https://example.com"
                    tooltipText="Hyperlink on the author's name"
                    bind:value={data.author.url}
                />
            </div>
        </Collapsible>
            ...
            <div slot="content" class="col-1">
                {#each data.fields as field, i}
                    ...
                    >
                        ...
                        />
                        <Checkbox
                            id={`embed-field-inline-${i}`}
                            label="Inline"
                            bind:value={field.inline}
                        />

                        <div
                            style="margin-top: 18px; display: flex; align-self: center"
                        >
                            <Button
                                danger
                                icon="fas fa-trash-can"
                                on:click={() => deleteField(i)}>Delete</Button
                            >
                        </div>
                    </div>
                    <div class="row">
                        <Textarea
                            col1
                            label="Field Value"
                            placeholder="Large text area, up to 1024 characters"
